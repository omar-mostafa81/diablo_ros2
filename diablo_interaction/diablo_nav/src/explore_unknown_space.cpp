...
//Modified the waypoint to have a vector of the yaws 
struct Waypoint {
    double x;
    double y;
...
{
public: 
    GoalPointsGenerator() : Node("explore_unknown_space"), x_c(0), y_c(0), yaw_c(0),
                            ...
                            last_heading_recieved_time_(this->now())
    {
        ...
        motion_cmd_subscriber_ = this->create_subscription<motion_msgs::msg::MotionCtrl>(
            "diablo/MotionCmd", 10,
            std::bind(&GoalPointsGenerator::MotionCmdCallback, this, std::placeholders::_1));
...
private:
    ...
    void MotionCmdCallback(const motion_msgs::msg::MotionCtrl::SharedPtr msg) {
        //If the robot is stationary or rotating around itself, don't accumulate the same yaw degree in the yaw vector
        double linear_v = msg->value.forward;
        double angular_v = msg->value.left;
        if ((linear_v == 0 && angular_v != 0) || (linear_v == 0 && angular_v == 0) || (linear_v < 0.06 && angular_v == 0)) {
    ...
    void explore()
    {
        if (goal_reached_ || !first_goal_received)
        {
            ...
            //Let time_to_wait be 10 sec for now to test the exploration strategy alone
            // if ((this->now() - last_goal_reached_time).seconds() > time_to_wait)
            if ((this->now() - last_heading_recieved_time_ ).seconds() > time_to_wait)
            {
    ...
    void startExploration()
    {
        while(!received_microphone_heading && exploration_called) {
            ...
            double exploration_direction = 0.0;
    ...
    rclcpp::Subscription<motion_msgs::msg::MotionCtrl>::SharedPtr motion_cmd_subscriber_;
    ...
    bool first_plane_created, rotating, pure_rotation_cmd;
    ...
    rclcpp::Time last_heading_recieved_time_;
